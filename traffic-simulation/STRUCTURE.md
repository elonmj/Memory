...
graph TB
    subgraph Project Structure
        ...
        subgraph Models
            ...
            LWR[lwr_model.py]
            MLWR[multiclass_lwr.py]
            FD[fundamental_diagram.py]
            VCM[vc_modulations.py]
            BaseModel --> LWR
            LWR --> MLWR
            FD --> LWR
            VCM --> MLWR
        ...
        subgraph Scenarios
            direction LR
            BS[base_scenario.py]
            MS[multiclass_scenarios.py]
            RL[red_light.py]
            SW[shock_wave.py]
            RW[rarefaction_wave.py]
            TJ[traffic_jam.py]
            BS --> RL & SW & RW & TJ & MS
        end
        ...
        subgraph Visualization
            direction LR
            P[plotter.py]
            FP[fundamental_plotter.py]
            MP[multiclass_plotter.py]
            DP[density_profile_plotter.py]
            SP[simulation_plotter.py]
            A[animator.py]
            P --> MP & SP
            SP --> DP
        end
        ...
        subgraph Analysis
            direction LR
            FCA[flow_capacity_analyzer.py]
        end
        
        subgraph Utils
            direction LR
            NM[numerical_methods.py]
        end

        subgraph Config
            direction LR
            SC[simulation_config.py]
        end
        
        subgraph Examples
            direction LR
            MIA[motorcycle_impact_analysis.py]
            MC[multiclass_comparison.py]
        end
        
        subgraph Outputs[simulations/]
            direction TB
            S[Simulations] --> |LWR & MultiClass| D & V & F & DI
            D[density/]
            V[velocity/]
            F[flow/]
            DI[diagrams/]
        ...
        subgraph Theoretical[Theoretical Framework]
            direction LR
            LWRTheory[LWR Model]
            MultiClass[Multi-Class Extension]
            Numerical[Numerical Methods]
            LWRTheory --> MultiClass
    ...
    %% Data flow
    Scenarios --> |Results| Visualization
    Visualization --> |Saves| Outputs
    Analysis --> |Results| Visualization

    classDef model fill:#f9f,stroke:#333,rx:5
    classDef scenario fill:#bbf,stroke:#333,rx:5
    classDef viz fill:#bfb,stroke:#333,rx:5
    classDef output fill:#fbb,stroke:#333,rx:5
    classDef theory fill:#bfb,stroke:#333,rx:5,stroke-dasharray: 5 5
    
    class BaseModel,LWR,MLWR,FD model
    class BS,MS,RL,SW,RW,TJ scenario
    class P,FP,MP,A viz
    class Outputs,S,D,V,F,DI output
    class LWRTheory,MultiClass,Numerical theory
```
